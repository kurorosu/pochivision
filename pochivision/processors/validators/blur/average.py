...
class AverageBlurValidator(BaseValidator):
    ...
    def validate_config(self, config: dict[str, Any]) -> None:
        ...
        kernel_size は長さ 2 の list/tuple で, 両要素とも 1 以上の int でなければならない.
        ...
        if not isinstance(kernel_size, (list, tuple)) or len(kernel_size) != 2:
            raise ProcessorValidationError(
                f"kernel_size must be a list/tuple of length 2, got {kernel_size!r}"
            )
        for v in kernel_size:
            ...
            if v < 1:
                raise ProcessorValidationError(
                    f"kernel_size elements must be >= 1, got {kernel_size!r}"
                )
